# Dev log (human): Pre-push dual dev log system
...
| **Date** | 2026-05-23 |
| **Time** | 16-54 |
| **Filename** | `005_2026-05-23_16-54_dev-log_pre-push-dual-dev-log-system.md` |
| **Agent audit** | [005_2026-05-23_16-54_dev-log-agent_pre-push-dual-dev-log-system.json](../agent/005_2026-05-23_16-54_dev-log-agent_pre-push-dual-dev-log-system.json) |
| **Git** | `unknown` @ `unknown` |
...
Every push now gets a **paired dev log**: a human markdown narrative and an agent JSON audit with the same timestamp. A generator (`npm run dev-log:pre-push`) auto-fills git state, test results, and a full repository tree; you fill decisions, trade-offs, and what changed. Consolidated models/prompts/file-structure also land in `file-exchange/exports/` for handoff.

---

## Goals and scope

- **In scope:** Pre-push dev-log tooling, templates, schema, Cursor command, AGENTS.md; consolidated exports in `file-exchange/exports/`; human-readable import/export stamps.
- **Out of scope:** Enforcing hook by default (optional sample only); migrating all legacy dev-log files.
...
|----|----------|-----------|------------------------|
| D1 | Human MD + agent JSON | Agents scan JSON faster; humans read prose | Single file type |
| D2 | Full tree embedded at push | Ground truth before push; no drift | Point to condense-file-structure only |
| D3 | `exports/consolidated-*.json` at exports root | Visible next to batch export folders | `models/` only |
...
- `backend/src/shared/utils/consolidatedExport.js` — write consolidated JSON to `file-exchange/exports/`
- `modelCondenser.service.js` mirrors to exports after condense
...
- `condense-prompts`, `condense-file-structure`, `condense:all` npm scripts
- Human-readable `formatExchangeTimestamp` (`2026-05-23_15-59-43Z`)

### Tooling / CI / docs

- `scripts/write-pre-push-dev-log.mjs`, `scripts/lib/*`
- `work-log/dev-logs/human/`, `agent/`, `templates/`, `schemas/`
- `.cursor/commands/pre-push-dev-log.md`, `AGENTS.md`, `dev-logs/README.md`
...
_Excludes `node_modules`, `.git`, `dist`._
...
**Stats:** 492 files, 314 directories.
